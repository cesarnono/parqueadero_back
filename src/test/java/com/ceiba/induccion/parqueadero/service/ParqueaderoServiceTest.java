...
public class ParqueaderoServiceTest {
	...
	public void verificarDisponibilidadServicioCarroTest() {
		...
		Servicio servicio = new ServicioTestDataBuilder().withDescripcion(ParqueaderoUtil.SERVICIO_PARQUEO_CARRO)
				.build();
		Servicio servicioEsperado = null;

		// Action
		when(servicioRepository.findByDescripcion(ParqueaderoUtil.SERVICIO_PARQUEO_CARRO))
				.thenReturn(new ServicioEntity(servicio));
	...
	public void verificarDisponibilidadServicioMotoTest() {
		...
		Servicio servicio = new ServicioTestDataBuilder().withDescripcion(ParqueaderoUtil.SERVICIO_PARQUEO_MOTO)
				.build();
		Servicio servicioEsperado = null;

		// Action
		when(servicioRepository.findByDescripcion(ParqueaderoUtil.SERVICIO_PARQUEO_MOTO))
				.thenReturn(new ServicioEntity(servicio));
		servicioEsperado = parqueaderoService.verificarDisponibilidadServicio(solicitudServicio);
	...
	public void registrarEntradaCarroTest() {
		SolicitudServicio solicitudServicio = new SolicitudServicio(ParqueaderoUtil.PLACA_COMUN, null,
				ParqueaderoUtil.SERVICIO_PARQUEO_CARRO, null, ParqueaderoUtil.getFecha());
		Servicio servicio = new ServicioTestDataBuilder().withDescripcion(ParqueaderoUtil.SERVICIO_PARQUEO_CARRO)
				.withSolicitudServicio(solicitudServicio).build();
	...
	/*
	 * @Test public void errorRegistrarEntradaTest() { Servicio servicio = null;
	 * CobroEntity cobroEsperado = null; cobroEsperado =
	 * parqueaderoService.registrarEntrada(servicio);
	 * Assert.assertTrue(cobroEsperado != null && cobroEsperado.getError() != null
	 * && ParqueaderoUtil.ERROR_REGISTRAR_ENTRADA.equals(
	 * cobroEsperado.getError())); }
	 */
	...
	public void registrarSalidaCarroCobroCincoHorasTest() {
		CobroCarro cobroCarro = new CobroTestDataBuilder().withFechaSalida(Calendar.getInstance())
				.withFechaEntrada(ParqueaderoUtil.restarHorasCalendar(Calendar.getInstance(), -5)).buildCarro();
		CobroEntity cobroEntity = new CobroEntity(cobroCarro);
		when(cobroRepository.findById(1L)).thenReturn(cobroEntity);
		doNothing().when(cobroRepository).delete(cobroEntity);
		when(cobroRepository.save(cobroEntity)).thenReturn(cobroEntity);
		...
		//
		Assert.assertTrue(
				cobroEsperado != null && cobroEsperado.getValorServicio() == (ParqueaderoUtil.TARIFA_HORA_CARRO * 5));
	}

	@Test
	public void registrarSalidaCarroCobroNueveHorasTest() {
		CobroCarro cobroCarro = new CobroTestDataBuilder().withFechaSalida(Calendar.getInstance())
				.withFechaEntrada(ParqueaderoUtil.restarHorasCalendar(Calendar.getInstance(), -9)).buildCarro();
		CobroEntity cobroEntity = new CobroEntity(cobroCarro);
		when(cobroRepository.findById(1L)).thenReturn(cobroEntity);
		doNothing().when(cobroRepository).delete(cobroEntity);
		when(cobroRepository.save(cobroEntity)).thenReturn(cobroEntity);
		...
		//
		Assert.assertTrue(
				cobroEsperado != null && cobroEsperado.getValorServicio() == (ParqueaderoUtil.TARIFA_DIA_CARRO * 1));
	}

	@Test
	public void registrarSalidaMotoCilindraje550CobroCincoHorasTest() {
		Cobro cobroMoto = new CobroTestDataBuilder().withFechaSalida(Calendar.getInstance())
	...
	public void registrarSalidaCarroCobroPocosMinutosTest() {
		CobroCarro cobroCarro = new CobroTestDataBuilder().withFechaSalida(Calendar.getInstance())
				.withFechaEntrada(ParqueaderoUtil.restarMinutosCalendar(Calendar.getInstance(), -8)).buildCarro();
		CobroEntity cobroEntity = new CobroEntity(cobroCarro);
		when(cobroRepository.findById(1L)).thenReturn(cobroEntity);
		doNothing().when(cobroRepository).delete(cobroEntity);
		when(cobroRepository.save(cobroEntity)).thenReturn(cobroEntity);
		...
		//
		Assert.assertTrue(
				cobroEsperado != null && cobroEsperado.getValorServicio() == (ParqueaderoUtil.TARIFA_HORA_CARRO * 1));
	}

	@Test
	public void crearCobroEntityConErrorTest() {
		String mensajeErrorTest = "Esto es un mensaje de error para test";
		CobroEntity cobroEntity = parqueaderoService.crearCobroEntityConError(mensajeErrorTest);
		assertTrue(cobroEntity != null && mensajeErrorTest.equals(cobroEntity.getError()));
	}
