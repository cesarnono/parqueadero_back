...
public class CobroTestDataBuilder {
	...
	private static final Calendar FECHA_ENTRADA = ParqueaderoUtil.getFechaCalendar("dd-M-yyyy HH:mm:ss", "18-03-2018 01:00:00");
	private static final Calendar FECHA_SALIDA =		ParqueaderoUtil.getFechaCalendar("dd-M-yyyy HH:mm:ss", "18-03-2018 06:10:00");
	...
	public CobroTestDataBuilder() {
		...
		this.estado = ESTADO;
		this.placa = PLACA;
		this.servicio = new ServicioTestDataBuilder().withDescripcion(ParqueaderoUtil.SERVICIO_PARQUEO_CARRO).withCupoMaximo(20).withTarifaDia(ParqueaderoUtil.TARIFA_DIA_CARRO).withTarifaHora(ParqueaderoUtil.TARIFA_HORA_CARRO).build();
		Date date1 = fechaEntrada.getTime();	
		Date date2 = fechaSalida.getTime();	
	}
	...
	public CobroCarro buildCarro() {
		return  new CobroCarro(-1,this.placa,this.fechaEntrada,this.fechaSalida,this.estado,this.valorServicio,this.tiempoServicioHoras,this.descripcionTiempoServicio,this.servicio);
	}
